...
> **📖 Documentação Completa**: [Animation Guide](../../docs/development/animations-guide.md)

## Visão Geral

O sistema de animações do TecnoJr usa **Framer Motion** com componentes reutilizáveis em `src/components/animated/`.

### Componentes Principais

- `FadeUp`, `FadeDown`, `FadeLeft`, `FadeRight` - Animações de entrada
- `ScaleIn` - Animação de escala
- `StaggerContainer` - Container para animações sequenciais
- `AnimatedCard`, `AnimatedButton` - Componentes especializados
