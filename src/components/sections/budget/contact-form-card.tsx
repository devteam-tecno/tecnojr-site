"use client";

import { MessageCircle, Shield } from "lucide-react";
import Link from "next/link";
import {
  AnimatedDiv,
  FadeIn,
  FadeUp,
  ScaleFadeIn,
} from "@/components/animated";
import { Button } from "@/components/ui/buttons/button";
import { getCardBackgroundClass } from "@/lib/utils/styles";

/**
 * Contact card component for budget section
 * Displays contact methods (WhatsApp and Email) with call-to-action
 */
export function ContactFormCard() {
  return (
    <FadeUp className="relative">
      {/* Card Glow Effect */}
      <div className="absolute -inset-1 rounded-3xl bg-linear-to-r from-tecno-purple-500 via-tecno-blue-500 to-tecno-purple-500 opacity-20 blur" />

      <div
        className={`relative rounded-3xl p-8 transition-all duration-500 hover:border-tecno-purple-500/50 lg:p-12 ${getCardBackgroundClass()}`}
      >
        {/* Header */}
        <div className="mb-8">
          <ScaleFadeIn
            delay={0.3}
            className="mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-linear-to-br from-tecno-purple-500 to-tecno-blue-500 shadow-lg shadow-tecno-purple-500/25"
          >
            <MessageCircle className="h-8 w-8 text-white" />
          </ScaleFadeIn>

          <h3 className="mb-3 text-2xl font-bold text-white lg:text-3xl">
            Vamos conversar?
          </h3>
          <p className="leading-relaxed text-gray-300">
            Conte-nos sobre seu projeto e receba uma proposta personalizada.
            Estamos aqui para transformar suas ideias em soluções incríveis!
          </p>
        </div>

        {/* Contact Methods */}
        <div className="grid grid-cols-2 gap-4">
          <AnimatedDiv hoverEffect="scale">
            <Button asChild variant="gradient-whatsapp" className="w-full">
              <Link
                href="https://wa.me/5573999502325"
                target="_blank"
                rel="noopener noreferrer"
              >
                <svg
                  aria-hidden="true"
                  className="mr-2 h-4 w-4"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                WhatsApp
              </Link>
            </Button>
          </AnimatedDiv>
          <AnimatedDiv hoverEffect="scale">
            <Button asChild variant="gradient-secondary" className="w-full">
              <Link href="/contato">
                <svg
                  aria-hidden="true"
                  className="mr-2 h-4 w-4"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                  />
                </svg>
                E-mail
              </Link>
            </Button>
          </AnimatedDiv>
        </div>

        {/* Security Badge */}
        <FadeIn
          delay={1}
          className="mt-6 flex items-center justify-center text-xs text-gray-400"
        >
          <Shield className="mr-2 h-4 w-4 text-tecno-purple-500" />
          Seus dados estão seguros conosco
        </FadeIn>
      </div>
    </FadeUp>
  );
}
